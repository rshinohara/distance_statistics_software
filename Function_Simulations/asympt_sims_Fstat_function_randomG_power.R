...
for (p in c(1/2,1/3)) {
	...
	### ASSESS SIMULATIONS
		...
		for (n in c(30,50,100,250)) {
			...
			### DO SIMULATIONS
			existing.files<-list.files(pattern=glob2rx(paste0('ganova_power_function_2018-12-19_n',n,'_sigma',sigma,'_tau1',tau1,'_tau2',tau2,'_p',round(p,2),'.RData')))
			if (length(existing.files)==0) {	
				system.time(sims<-mclapply(rep(n,B),do.simulation,p,tau1,tau2,sigma,ise,mc.cores=n.cores))
				save(sims,file=paste('ganova_power_function_2018-12-19_n',n,'_sigma',sigma,'_tau1',tau1,'_tau2',tau2,'_p',round(p,2),'.RData',sep=''))
			} else {
	...
	save(power,adonis.power,time.proposed,time.competitor,file=paste0('ganova_power_2018-12-19_power_sims_function_p',round(p,2),'_sigma',sigma,'.RData'))
}
