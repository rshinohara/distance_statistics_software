...
do.proposed.test<-function(n,d,dd,n.1,n.2,d.mat,B.mc,exact=FALSE,return.details=FALSE,return.sample=TRUE,return.sample.size=5000) {
...
if (!file.exists('CURE_DTI_2017-10-18.RData')) {
...
existing.files<-list.files(pattern=glob2rx(paste0('ganova_simulations_dti_2018-11-28_n',n,'_mu',mu,'_k',ks[k],'.RData')))
if (length(existing.files)==0) {	
	system.time(sims<-mclapply(rep(n,B),do.simulation,mu,k,vec.d,mc.cores=n.cores))
	save(sims,file=paste('ganova_simulations_dti_2018-11-28_n',n,'_mu',mu,'_k',ks[k],'.RData',sep=''))
} else {
