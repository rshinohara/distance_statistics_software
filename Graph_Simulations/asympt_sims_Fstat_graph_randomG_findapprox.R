...
#	October 18, 2017
#	bsub < asympt_sims_Fstat_graph_randomG_findapprox.sh
#################################################
...
do.simulation<-function(n,p,tau,d,exact=FALSE) {
	...
	proposed.test<-do.proposed.test(n=n,d=d,dd=simd.data$dd,n.1=simd.data$n.1,n.2=simd.data$n.2,sample.g1=simd.data$sample.g1,sample.g2=simd.data$sample.g2,d.mat=d.mat,exact=exact)
...
type1err.proposed<-apply(matrix(unlist(lapply(sims,function(x) x$proposed.test.p)),nrow=B,byrow=TRUE)<0.05,2,mean,na.rm=TRUE)
type1err.competitor<-apply(matrix(unlist(lapply(sims,function(x) x$adonis.p)),nrow=B,byrow=TRUE)<0.05,2,mean,na.rm=TRUE)
...
save(sims,type1err.proposed,type1err.proposed,p.err.proposed,p.err.competitor,time.proposed,time.competitor,file='ganova_graph_2017-10-18_approx_times.RData')

cols<-brewer.pal(3,'Dark2')

pdf('approx_time_graph_n250_2017-10-18.pdf',height=7,width=10)
par(mfrow=c(1,2))
