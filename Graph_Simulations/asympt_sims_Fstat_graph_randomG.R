...
#	December 28, 2018
#	bsub < asympt_sims_Fstat_graph_randomG.sh
#################################################
...
do.simulation<-function(n,p,tau,d,B.mc=2.5E5,exact=FALSE) {
	
	simd.data<-simulate.data(n,p,tau,d) 
	...
	proposed.test.time<-system.time(proposed.test<-do.proposed.test(n=n,d=d,dd=simd.data$dd,n.1=simd.data$n.1,n.2=simd.data$n.2,sample.g1=simd.data$sample.g1,sample.g2=simd.data$sample.g2,d.mat=d.mat,B.mc=B.mc,exact=exact,return.details = TRUE))[[3]]+proposed.dmat.time
...
for (p in c(1/2,1/3)) {
	...
	type1err.proposed<-array(NA,dim=c(3,4))
	type1err.competitor<-array(NA,dim=c(3,4))
	time.proposed<-array(NA,dim=c(3,4))
	time.competitor<-array(NA,dim=c(3,4))
	
	dimnames(type1err.proposed)[[1]]<-paste0('tau=',c(0.1,0.15,0.2))
	dimnames(type1err.proposed)[[2]]<-paste0('n=',c(30,50,100,250))

	dimnames(type1err.competitor)[[1]]<-paste0('tau=',c(0.1,0.15,0.2))
	dimnames(type1err.competitor)[[2]]<-paste0('n=',c(30,50,100,250))

	dimnames(time.proposed)[[1]]<-paste0('tau=',c(0.1,0.15,0.2))
	dimnames(time.proposed)[[2]]<-paste0('n=',c(30,50,100,250))

	dimnames(time.competitor)[[1]]<-paste0('tau=',c(0.1,0.15,0.2))
	...
	for (tau in c(0.1,0.15,0.2)) {
		...
		for (n in c(30,50,100,250)) {
			...
			set.seed(976741)
			### DO SIMULATIONS
			existing.files<-list.files(pattern=glob2rx(paste0('ganova_type1err_graph_2018-12-19_n',n,'_tau',tau,'_p',round(p,2),'.RData')))
			if (length(existing.files)==0) {	
				system.time(sims<-mclapply(rep(n,B),do.simulation,p,tau,edge.diff,mc.cores=n.cores))
				save(sims,file=paste('ganova_type1err_graph_2018-12-19_n',n,'_tau',tau,'_p',round(p,2),'.RData',sep=''))
			} else {
				load(existing.files[length(existing.files)])
			}
			
			#which(!sapply(sims, function(x) !inherits(x, "try-error")))

			try(type1err.proposed[i1,i2]<-mean(unlist(lapply(sims,function(x) x$proposed.test.p))<0.05,na.rm=TRUE))
			try(type1err.competitor[i1,i2]<-mean(unlist(lapply(sims,function(x) x$adonis.p))<0.05,na.rm=TRUE))
	...
	print(type1err.proposed)
	print(type1err.competitor)
	print(time.proposed)
	print(time.competitor)
	save(type1err.proposed,type1err.competitor,time.proposed,time.competitor,file=paste0('ganova_2018-12-19_type1err_sims_p',round(p,2),'_graphs.RData'))
}