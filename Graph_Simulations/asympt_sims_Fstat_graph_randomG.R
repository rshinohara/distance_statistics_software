...
for (p in c(1/2,1/3)) {
	...
	for (tau in c(0.1,0.15,0.2)) {
		...
		for (n in c(30,50,100,250)) {
			...
			if (length(existing.files)==0) {	
				...
				save(sims,file=paste('ganova_type1err_graph_2018-11-28_n',n,'_tau',tau,'_p',round(p,2),'.RData',sep=''))
			} else {
	...
	save(type1err.proposed,type1err.competitor,time.proposed,time.competitor,file=paste0('ganova_2018-11-28_type1err_sims_p',round(p,2),'_graphs.RData'))
}